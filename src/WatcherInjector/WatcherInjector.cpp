// WatcherInjector – DLL-Worker: NUR eine JSON-Datei schreiben:
// BASE\session_stats\live.session.json
//
// Änderungen:
// - write_json() schreibt ausschließlich live.session.json
// - KEIN Spiegeln mehr nach BASE\live_control.json
// - KEIN Schreiben von session_stats\live_control.json
...
// Wert-Extraktion (Big-Endian-Aufbau),
...
static void write_json(const std::wstring& base, const std::wstring& rom, int cp, int pc, int cb, int bp) {
    const fs::path out = fs::path(base) / L"session_stats" / L"live.session.json";
...
// --- Worker-Thread der DLL ---
// Pollt NVRAM und schreibt alle ~200ms die live.session.json
    ...
    while (g_running) {
        ...
        if (read_bytes(cfg.nvram, buf)) {
            ...
            for (const auto& f : cfg.fields) {
                const int v = extract_value(buf, f);
...
BOOL APIENTRY DllMain(HMODULE hModule,
    ...
    case DLL_PROCESS_ATTACH: {
        ...
        HANDLE hThread = CreateThread(nullptr, 0, Worker, nullptr, 0, nullptr);
